...
def validate_input(key, raw, rules):
    ...
    if rule["type"] == "int":
        try:
            value = int(stripped_input)
        except:
            return (False, None, "Please enter a valid integer.")

        if "min" in rule:
            if value < rule["min"]:
                return (False, None, f"Please enter a number greater than {rule['min']}")
        if "max" in rule:
            if value > rule["max"]:
                return (False, None, f"Please enter a number less than {rule['max']}")
    ...
    if rule["type"] == "word":
        if stripped_input == "":
            return False, None, "Can not enter an empty input"
        if " " in stripped_input:
            return False, None, "Can not enter a word with spaces"
        if stripped_input.isdigit():
            return False, None, "Can not enter a number"
...
def collect_answers(prompts, rules):
    ...
    for key, value in prompts.items():
        while True:
            ...
            ok, value, error_message = validate_input(key, user_prompt, rules)
            if ok:
                answers[key] = value
                break
            else:
